...
import { getAllStages } from "@/functions/step.query"
import Stages from "@/components/stages/Stages"
export default function Cursos(){
    const [step,setStages]=useState<stage[]>([
        {
            id: '0',
            name: '',
            description: '',
    ...
    const colors:string[]=['#FF0303','#FF0303','#63CAA7','#63CAA7','#9510B8','#9510B8','#FF0303','#FF0303','#63CAA7','#63CAA7','#9510B8','#9510B8']
    return(
        <div id="CursosPage" className={`${styles.Container} min-h-[80vh] flex flex-col items-center bg-white md:gap-8`} >
          ...
          <section className="h-[10rem] bg-blue relative w-[99%] flex items-center justify-center text-white  2xl:text-2xl  xl:text-6xl lg:text-lg  sm:text-sm ">
             ...
             <Image src={image4} alt='cjimg'  className=" w-[auto] absolute bottom-[2.7rem]  right-6  h-[3rem] "/>
             <Image src={image3} alt='cjimg'  className=" w-[auto] absolute bottom-[3.2rem]  right-[1.2rem] h-[3rem]  min-[400px]:h-[3.5rem] "/>
             <Image src={image3} alt='cjimg'  className=" w-[auto] absolute bottom-4 right-[5rem] h-[3rem]  min-[400px]:h-[3.5rem] "/>
             </div>
         </section>
         ...
         <section className={`${styles.CoursesSection}   h-full overflow-auto min-[300px]:h-auto w-full flex  justify-center text text-xs
             ...
             <div className=" flex    h-5/6 md:h-full  w-full items-center  flex-col min-[300px]:flex-row  sm:justify-evenly min-[400px]:w-full  sm:w-11/12 ">
                ...
                <div className="grid grid-cols-2 h-full md:gap-8 grid-flow-row-dense  w-full  md:w-[40rem] xl:w-[43rem] ">
                    {step.map((item,key)=>
                    <Stages id={item.id} key={key}  name={item.name} maxAge={item.maxAge} 
                    minAge={item.minAge} color={colors[key]} description={item.description} 
                    />
                    )}
                </div>
